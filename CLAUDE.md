...
Fehlt ein Adapter im geöffneten Root, zuerst die geerbten MCP-Konfigurationen prüfen und pro Client nur eine Registrierung verwenden. Eine zusätzliche STDIO-Registrierung nutzt `P:\LocalRAG\.venv\Scripts\python.exe -m local_rag.adapter --project-id <knowledge_system|copulaantrag|thermoexpress|mbintra>` (Arbeitsverzeichnis `P:\LocalRAG`). Die venv-Startdatei und ihr System-Python-Kindprozess sind normal, kein zweiter Stack. Diagnose: `& P:\LocalRAG\scripts\status.ps1 -Deep` prüft alle Quellen und kann deutlich länger als das MCP-Startfenster dauern; Runtime-Drift prüft `& P:\LocalRAG\.venv\Scripts\python.exe -m local_rag.runtime_update --runtime-root $env:LOCALAPPDATA\LocalRAG`.
