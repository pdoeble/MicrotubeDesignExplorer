...
Die Anwendung besteht gemäß ADR-0008 und ADR-0009 aus vier Workflow-Tabs: **Start**, **Model Setup**, **Results**, **Settings**. Model Setup bündelt die wissenschaftlichen Eingaben in fünf lokale Fachtabs: Geometry, Solid material, Air circuit, Coolant circuit und Screens & boundaries. In jedem Fachtab wird genau eine Gruppenkopplung sowie genau eine ausgewählte Reference- oder Comparison-Konfiguration gezeigt; verknüpfte Comparison-Eingaben werden nicht als identische Formulare wiederholt. Die Tabs werden beim Initialisieren vorgeladen, damit die Aktivierung ohne merkliche Latenz funktioniert; genau dafür empfiehlt das WAI-ARIA Tabs Pattern automatische Aktivierung auf Fokus nur dann, wenn die Inhalte ohne spürbare Verzögerung bereitstehen. Diese Bedingung ist hier erfüllbar, weil die Seitenstruktur statisch ist und nur die Rechenergebnisse asynchron nachgeladen werden. citeturn15view0
...
In **Model Setup** werden alle geometrischen, betrieblichen und randbedingungsbezogenen Eingaben in ihrem jeweiligen Fachtab geführt. Standardmäßig ist der im Paper beschriebene Vergleichsfall aktiv, also gleiche Geometrie mit Materialvariation, aber die Oberfläche lässt auch vollständig unterschiedliche Geometrien zu. Kopplungen werden **gruppenweise** gesteuert, nicht global. Pflichtgruppen sind: `geometry`, `materials`, `air_side`, `coolant_side`, `boundary_conditions`. Der Umschalter Same values/Separate values liegt jeweils direkt im aktiven Fachtab; daneben wechselt Reference/Comparison den sichtbaren Entwurf. Jeder editierbare Parameter wird über dieselbe UI-Struktur abgebildet: Slider, direkt editierbares Zahlenfeld, Einheit, Reset auf Paper-Default. Ob ein Parameter linear oder logarithmisch gesteuert wird, wird **nicht** ad hoc im UI entschieden, sondern in einem zentralen **Parameter Manifest** festgeschrieben. Dasselbe Manifest definiert Default, Minimum, Maximum, Einheit, Rundung, Ganzzahligkeit und Sichtbarkeitsbedingungen. citeturn17view0turn20view0
...
In **Model Setup** werden alle Stoffwerte, die tatsächlich in die Berechnung eingehen, in den fünf fachlichen Tabs Geometry, Solid material, Air circuit, Coolant circuit und Screens & boundaries geführt. Es gibt **keine Datenbank**, nur explizite Eingabefelder mit Paper-Defaults. Fluide und Materialien werden gleich behandelt: als statische Eigenschaftssätze ohne temperaturabhängige Stoffmodelle. Jeder Tab schaltet zwischen Reference und Comparison sowie zwischen gleichen und separaten Werten um. Ist eine Gruppe gekoppelt, zeigt die Comparison-Ansicht einen eindeutigen Link-Status statt eines zweiten identischen Formulars; „Edit separately“ stellt die vorherigen unabhängigen Werte wieder her. Jedes Feld braucht Name, Symbol, Einheit, Default, zulässigen Bereich und eine Kurzbeschreibung der Wirkung im Modell.
